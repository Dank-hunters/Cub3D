...
/*   Updated: 2021/06/20 15:46:57 by cguiot           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
{
	map->xpm[0].img = mlx_xpm_file_to_image(map->img.mlx, map->pt_no_t, &map->xpm[0].width, &map->xpm[0].height);
	map->xpm[1].img = mlx_xpm_file_to_image(map->img.mlx, map->pt_so_t, &map->xpm[1].width, &map->xpm[1].height);
	map->xpm[2].img = mlx_xpm_file_to_image(map->img.mlx, map->pt_ea_t, &map->xpm[2].width, &map->xpm[2].height);
	map->xpm[3].img = mlx_xpm_file_to_image(map->img.mlx, map->pt_we_t, &map->xpm[3].width, &map->xpm[3].height);
}
...
int	init_text(t_info *map)
{
	...
	while(i <= 3)
	{
		if (map->xpm[i].img == NULL)
        {
			...
			map->xpm[i].addr = (int *)mlx_get_data_addr(map->xpm[i].img, &map->xpm[i].bpp, &map->xpm[i].line_len,
						&map->xpm[i].endian);
